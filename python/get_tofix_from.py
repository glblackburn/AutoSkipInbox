...
keep_inbox_label_id = labels_name_map.get("keep-inbox")
autoskip_label_id = labels_name_map.get("autoskipinbox")
to_fix_label_id = labels_name_map.get("autoskipinbox/tofix")
...
# List messages with autoskipinbox-tofix label
result = service.users().messages().list(
    userId=user_id, q="label:autoskipinbox-tofix"
).execute()
...
if messages:
    for message in messages:
        ...
        modify_request = {
            'addLabelIds': [keep_inbox_label_id, inbox_id],
            'removeLabelIds': [autoskip_label_id, to_fix_label_id]
        }
...
for email in from_list.keys():
    ...
    if filter_list.get(email, 0) == 0:
        # Create filter
        filter_action = {
            'addLabelIds': [keep_inbox_label_id]
        }
        filter_criteria = {
            'from': email
        }
